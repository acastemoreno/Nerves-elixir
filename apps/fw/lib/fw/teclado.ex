defmodule Fw.Teclado do
  ...
  def start_link(gpio_output, gpio_input) do
    GenServer.start_link(__MODULE__, [gpio_output, gpio_input], name: __MODULE__)
  end
  ...
  def init([{output1, output2, output3, output4}, {input1, input2, input3, input4}]) do
  ...
  defp siguiente?(0, estado, longitud_texto, _paso_actual, siguiente_paso, {_last_hit, _letra}) do  ## NO detecta ninguna letra, pasa a la siguiente fila
    {:reply, {:procesando, nil}, estado |> Map.put(:estado, :procesando)
                                          |> Map.put(:paso, {siguiente_paso, longitud_texto})
                                          |> Map.put(:last_hit, nil)}
  end

  defp siguiente?(1, estado, longitud_texto, paso_actual, _siguiente_paso, {last_hit, letra}) when last_hit == letra do ## SI detecta letra, es el ultimo hit, sigue en paso actual, longitud igual
    {:reply, {:procesando, nil}, estado |> Map.put(:estado, :procesando)
                                        |> Map.put(:paso, {paso_actual, longitud_texto})}
  end

  defp siguiente?(1, estado, 5, _paso_actual, _siguiente_paso, {_last_hit, letra}) do  ## SI detecta letra, no es el ultimo hit, sigue en paso actual, longitud + 1
    {:reply, {:terminado, letra}, estado |> Map.put(:estado, :disponible)
                                          |> Map.put(:paso, {:o1, 0})
                                          |> Map.put(:last_hit, nil)}
  end

  defp siguiente?(1, estado, longitud_texto, paso_actual, _siguiente_paso, {_last_hit, letra}) do  ## SI detecta letra, no es el ultimo hit, sigue en paso actual, longitud + 1
